...
func (fd *zeroFD) ConfigureMMap(ctx context.Context, opts *memmap.MMapOpts) error {
	if opts.Private || !opts.MaxPerms.Write {
		...
		// Linux terms, it isn't VM_SHARED), so implement it as a private
		// anonymous mapping, but back it with fd; this is what Linux does, and
		// is actually application-visible because the resulting VMA will show
		// up in /proc/[pid]/maps with fd.vfsfd.VirtualDentry()'s path rather
		// than "/dev/zero (deleted)".
		opts.Offset = 0
